...
from edu.stanford.cmed.devcli.model.RepoCheckEntry import RepoCheckEntry
...
FS_TYPE_DIR = '📁 dir'
FS_TYPE_FILE = '📄 file'
RECOGNIZED_AS_APP_REPO = APP_NAME + ' repo'
STATUS_ICON_OK = '✅'
STATUS_ICON_UNKNOWN = '❓'
STATUS_ICON_MISSING = '❌'
STATUS_OK = 'ok'
...
KNOWN_FS = {
    'archive': 'Known ' + APP_NAME + ' archive folder',
    '.DS_Store': 'Known mac file',
    'set-env.sh': 'Known ' + APP_NAME + ' shell script',
}
...
class RepoWorker(Worker):
    ...
    def analyze_entry(fs_name, repo_map):
        ...
        if fs_name in repo_map:
            recognized_as = RECOGNIZED_AS_APP_REPO
            status = STATUS_OK
    ...
    def check_repos():
        ...
        dir_list = os.listdir(Util.app_home)
        for fs_name in dir_list:
            full_path = os.path.join(Util.app_home, fs_name)
