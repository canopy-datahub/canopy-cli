...
# Required ones must come from either .env.${CANOPY_ENV} (generated by
# `canopycli init ui-env`) or the shell env; the rest are optional.
...
class BuildEcsWorker:
    ...
    def _ui_build_args(ctx: dict) -> Dict[str, str]:
        env_file = ctx["service_path"] / f".env.{ctx['env']}"
        ...
        if missing:
            raise _DeploymentError(
                ...
                f"\n    Generate the file with: canopycli init ui-env"
            )
