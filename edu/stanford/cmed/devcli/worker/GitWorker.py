...
from edu.stanford.cmed.devcli.util.GlobalContext import GlobalContext, UTF_8
from edu.stanford.cmed.devcli.util.RepoResultTriple import RepoResultTriple
from edu.stanford.cmed.devcli.util.ResultTable import ResultTable
...
class GitWorker(Worker):
    ...
    def execute_shell_on_all_repos_with_table(command_list,
        ...
        with Progress() as progress:
            ...
            for repo in repo_list:
                ...
                return_code = -1
                try:
                    cwd = Util.get_wd(repo) if cwd_is_home is False else Util.app_home
    ...
    def next(self):
        ...
        if len(active_repos) > 0:
            last_repo_path = Util.read_app_specific_file('last_git_repo')
            found_idx = -1
            ...
            Util.write_app_specific_file(Util.LAST_GIT_FILE, path + "\n")
            Util.write_app_specific_file(Util.NEXT_GIT_FILE, path + "\n")
        else:
            Util.delete_app_specific_file(Util.LAST_GIT_FILE)
            Util.delete_app_specific_file(Util.NEXT_GIT_FILE)
